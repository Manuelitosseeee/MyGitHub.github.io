...
import { SKINS, skinBaseFont } from "../data/skins";
import { SectionTitle, Card, Switch, Confirm, Seg, toast } from "../ui/primitives";
import { cx } from "../lib/utils";
import type { Appearance, FontChoice, FontSize, ThemeAccent } from "../data/types";
...
/* Carattere options. "auto" follows the base font of the Aspetto Totale
   design currently active; the concrete families override it everywhere. */
const FONT_OPTIONS: Array<{ id: FontChoice; label: string; desc: string }> = [
  {
    id: "auto",
    label: "Automatico",
    desc: "Segue il carattere del design scelto in Aspetto Totale",
  },
  { id: "system", label: "Classico", desc: "SF Pro / System — pulito e nativo" },
  {
    id: "rounded",
    label: "Arrotondato",
    desc: "SF Rounded — morbido e amichevole",
  },
  ...
  { id: "mono", label: "Tecnico", desc: "Monospaziato, cifre allineate" },
  {
    id: "sans",
    label: "Geometrico",
    desc: "Avenir Next — moderno e geometrico",
  },
];
...
const FONT_STACKS: Record<Exclude<FontChoice, "auto">, string> = {
  system:
    '-apple-system, BlinkMacSystemFont, "SF Pro Text", "Segoe UI", Roboto,"Helvetica Neue", sans-serif',
  rounded:
    'ui-rounded, "SF Pro Rounded", "Hiragino Maru Gothic ProN", -apple-system, BlinkMacSystemFont, "Segoe UI", "Helvetica Neue", sans-serif',
  serif:
    '"Iowan Old Style", "Palatino Linotype", Palatino, Georgia,"Times New Roman", serif',
  mono:
    'ui-monospace, "SF Mono", Menlo, Monaco, "Cascadia Code","Roboto Mono", monospace',
  sans:
    '"Avenir Next", Avenir, "Century Gothic", "Trebuchet MS", -apple-system, BlinkMacSystemFont, "Segoe UI", "Helvetica Neue", sans-serif',
};
    ...
    <div className="screen">
      ...
      <Card className="card-pad">
        <p className="row-sub" style={{ marginBottom: 14 }}>
          Dieci design completi: cambiano colori, forme dei riquadri e, se il
          carattere è su “Automatico”, anche il tipo di testo. Testi, funzioni
          e posizione degli elementi restano uguali.
        </p>
        <div className="skin-grid">
          {SKINS.map((s) => (
            ...
            >
              <span
                className="skin-tile"
                style={{ background: s.prev.bg, color: s.prev.text }}
              >
                <span
                  className="st-panel"
                  style={{ background: s.prev.card, borderRadius: 9 }}
                >
                  <span className="st-title" />
                  <span className="st-line" />
                  <span className="st-bar" />
                  <span className="st-line" style={{ width: "70%" }} />
                </span>
                <span
                  className="st-font"
                  style={{
                    background: s.prev.card,
                    borderRadius: Math.min(12, s.prev.radius / 2 + 2),
                  }}
                >
                  <b style={{ fontFamily: FONT_STACKS[s.baseFont] }}>Aa</b>
                  <i style={{ fontFamily: FONT_STACKS[s.baseFont] }}>120</i>
                </span>
                {ap.skin === s.id ? (
              ...
              <span className="skin-tag">{s.tagline}</span>
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card className="card-pad">
        ...
        <div className="row-title" style={{ fontSize: 15, marginBottom: 8 }}>
          <TextCursorInput size={15} style={{ verticalAlign: "-2px" }} />{" "}
          Carattere
        </div>
        <p className="row-sub" style={{ marginBottom: 12 }}>
          Con “Automatico” ogni Aspetto Totale usa il suo carattere; scegliendo
          uno degli altri, resta lo stesso in tutti i design.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {FONT_OPTIONS.map((f) => {
            /* For "auto" preview what the ACTIVE skin would use. */
            const stack =
              f.id === "auto"
                ? FONT_STACKS[skinBaseFont(ap.skin)]
                : FONT_STACKS[f.id];
            return (
              <button
                key={f.id}
              ...
              >
                <span style={{ flex: 1, textAlign: "left", fontFamily: stack }}>
                  ...
                  <span
                    ...
                    style={{
                      ...
                      marginTop: 6,
                      fontSize: 15,
                      letterSpacing: f.id === "mono" ? "-0.01em" : undefined,
                    }}
                  >
                    120 BPM · AaBbCc 0123
                  </span>
